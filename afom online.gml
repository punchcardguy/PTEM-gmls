gml with(instance_create(0, 0, obj_custom_object_ext))
{
	ini_open("online.ini");
	name = ini_read_string("data", "name", NAME);
	system_font = IS_SYSTEM_FONT;
	chat_font = CHAT_FONT;
	host = ini_read_string("data", "host", SERVER);
	port = ini_read_real("data", "port", SERVER_PORT);
	ini_close();
	global._chat = undefined;
	namedialog = undefined;
	hostdialog = undefined;
	portdialog = undefined;
	chat_history = [];
	chat_bind = CHAT_BIND;
	...
	client_id = undefined;
	ws = network_create_socket(network_socket_ws);
	network_set_timeout(ws, 4000, 4000);
	
	depth = obj_camera.depth -1;
	persistent = 1;
	q = -1;
	sprite_index = sprite_exists(asset_get_index("spr_COMMUNITYicon")) ? spr_COMMUNITYicon : spr_null;
	x = x - bbox_left;
	y = y - bbox_top;
	yi = 600;
	showtext = 0;
	...
	playerdraw = @'
		...
		if(is_self)
		{
			...
			draw_text(x, bbox_top - 20, name);
			draw_set_color(c_white);
		}
		else if(room_ind == real(room))
		{
		...
		var message = messages[i];
			if(is_self || room_ind == real(room))
			{
				draw_set_alpha(message[1]);
				draw_text(x, bbox_top - 36 - (16 * i), message[0]);
			}
			message[1] -= 0.002 / min(0.1 * string_length(message[0]), 2);
			if(message[1] <= 0)
	...
	playerdraw_saved = live_snippet_create(playerdraw);
	event.room_start[0] = @'
		if(room == rank_room)
			alarm[6] = 725;
		if(room == tower_3)
		{
			with(instance_nearest(2880, 2400, obj_platform))
			{
				x += 128;
				image_xscale = 6;
			}
		}
		if(string_pos("tower_", room_get_name(room)) == 1)
		{
			with(obj_solid)
				visible = true;
			with(obj_slope)
			...
			with(obj_startgate)
				level = (targetRoom == street_intro) ? "street" : string_letters(room_get_name(targetRoom));
		}
		if !levelLoaded
			exit;
	...
	event.alarms[0][0] = @'with(obj_custom_object)
		{
			if(variable_instance_exists(id, "room_ind"))
				instance_destroy();
		}
	
		if(system_font && font_exists(chat_font))
			font_delete(chat_font);
		
		network_destroy(ws);
		array_push(notifications, 
		{
			text : irandom_range(1,100) == 1 ? "Connection terminated. I`m sorry to interrupt you, Elizabeth, if you still even remember that name, But I`m afraid you`ve been misinformed. You are not here to receive a gift, nor have you been called here by the individual you assume, although, you have indeed been called. You have all been called here, into a labyrinth of sounds and smells, misdirection and misfortune. A labyrinth with no exit, a maze with no prize. You don`t even realize that you are trapped. Your lust for blood has driven you in endless circles, chasing the cries of children in some unseen chamber, always seeming so near, yet somehow out of reach, but you will never find them. None of you will. This is where your story ends. And to you, my brave volunteer, who somehow found this job listing not intended for you, although there was a way out planned for you, I have a feeling that`s not what you want. I have a feeling that you are right where you want to be. I am remaining as well. I am nearby. This place will not be remembered, and the memory of everything that started this can finally begin to fade away. As the agony of every tragedy should. And to you monsters trapped in the corridors, be still and give up your spirits. They don`t belong to you. For most of you, I believe there is peace and perhaps more waiting for you after the smoke clears. Although, for one of you, the darkest pit of Hell has opened to swallow you whole, so don`t keep the devil waiting, old friend. My daughter, if you can hear me, I knew you would return as well. It`s in your nature to protect the innocent. I`m sorry that on that day, the day you were shut out and left to die, no one was there to lift you up into their arms the way you lifted others into yours, and then, what became of you. I should have known you wouldn`t be content to disappear, not my daughter. I couldn`t save you then, so let me save you now. It`s time to rest - for you, and for those you have carried in your arms. This ends for all of us. End communication." : "Connection terminated.",
			duration : 60,
			maxduration : 60, 
			x : -340,
			y : 0
	...
	event.alarms[1][0] = @'instance_destroy()';
	// alarms 2 and 3 don't function due to a programming oversight on my part. oops. -main_thing
	event.alarms[4][0] = @'can_connect=true;'
	
	event.alarms[5][0] = @'
		instance_destroy(obj_peppinoselect);
		instance_destroy(obj_levelselect);
	...
	event.alarms[6][0] = @'
		...
		scr_playerreset();
		audio_stop_sound(obj_music.musicID);
		obj_music.musicID = scr_music(mu_hub);
	...
	event.alarms[7][0] = @'obj_player1.x = tpx;obj_player1.y = tpy'
	netbuff = buffer_create(0, buffer_grow, 4);
	buffer_seek(netbuff, buffer_seek_start, 0);
	buffer_write(netbuff, buffer_u32, 0 /*TYPE_UPDATE*/);
	notifications = [];
	config_option = function(_text, variablename) constructor
	{
		text = _text;
		variable = variablename;
		variabledialog = variablename + "dialog";
	} 
	options = [
		new config_option("Name", "name"), 
		new config_option("Host", "host"), 
		new config_option("Port", "port")
	];
	event.step[0] = @'
		if(showtext == 1)
		{
		...
		with(obj_player1)
		{
			if(state == 84)
			{
				if(key_up2)
				{
					if(character == "P" && ispeppino == 1)
					{
						ispeppino = 0;
						paletteselect = 0;
					}
					else if(character == "P" && ispeppino == 0)
					{
						scr_soundeffect(sfx_Npogo2);
						character = "N";
						paletteselect = 0;
					}
					else if(character == "N")
		...
		if(connect_time > -1)
		{
			if(chat_bind != noone)
			{
				if(keyboard_check_pressed(chat_bind))
				{
					global._chat = get_string_async("chat", "");
					with(obj_player1)
					{
						state = 1;
			...
			if(q <= 18)
			{
				...
				with(obj_player1)
				{
					buffer_write(netbuff, buffer_f32, x);
					buffer_write(netbuff, buffer_f32, y);
					buffer_write(netbuff, buffer_u32, (xscale > 0 ? 1 : 0)); // for some reason buffer_bool, buffer_u8, and buffer_u32 are the same size. why???
					buffer_write(netbuff, buffer_u32, real(sprite_index));
					buffer_write(netbuff, buffer_f32, image_index);
					buffer_write(netbuff, buffer_u32, real(room));
					buffer_write(netbuff, buffer_u32, real(spr_palette));
					buffer_write(netbuff, buffer_u32, paletteselect);
					buffer_write(netbuff, buffer_f32, angle);
				}
			}
			else
			{
				...
				if(buffer_get_size(netbuff) == 1024)
					network_send_raw(ws, netbuff, buffer_get_size(netbuff), network_send_binary);
				buffer_delete(netbuff);
			...
			var _keys = ds_map_keys_to_array(clients);
			for(var i = 0; i < array_length(_keys); i++)
			{
				var client = clients[? _keys[i]];
				with(client[0])
				{
					if(is_self)
						continue;
	...
	event.draw_gui[0] = @'
	draw__button = function(_x, _y, outline_color = c_white)
	{
		var hovering = point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), _x, _y, _x + 181, _y + 47) && device_mouse_check_button(0, mb_left);
		
		draw_set_color(hovering ? #363636 : c_black);
		draw_rectangle(_x, _y, _x + 181, _y + 47, false); // button background
		draw_set_color(outline_color);
		draw_rectangle(_x, _y, _x + 181, _y + 47, true); // button outline 
	}
	...
	if(connect_time == -1)
	{
		...
		draw_set_alpha(0.6);
		draw_rectangle(256, 0, 687, display_get_gui_height(), false); // background
		draw_set_alpha(1);
		
		draw_line(256, 0, 256, display_get_gui_height());
		draw_line(687, 0, 687, display_get_gui_height());
	
		/* draw_text(190, 150, "NAME");
		draw_text(190, 250, "HOST");
		draw_text(190, 350, "PORT");
		
		draw_roundrect_ext(190, 150, 490, 190, 10, 10, false); // name background 
		draw_roundrect_ext(190, 250, 490, 290, 10, 10, false); // host background 
		draw_roundrect_ext(190, 350, 490, 390, 10, 10, false); // port background */
		...
		if !credits
		{
			for(var i = 0, len = array_length(options); i < len; i++) 
			{
				...
				draw_text(320, _y + (i * _ypad), options[i].text);
				draw_set_color(c_black);
				draw_rectangle(295, 132 + (i * 63), 646, 155	+ (i * _ypad), true); // outline
				draw_rectangle(294, 131 + (i * 63), 647, 156	+ (i * _ypad), true); // outline
				draw_set_color(c_white);
				draw_rectangle(296, 133 + (i * 63), 645, 154	+ (i * _ypad), false);
				
				draw_set_halign(fa_left);
				draw_set_color(c_black);
				draw_set_font(font0);
				
				draw_text_transformed(300, 152 + (i * _ypad), variable_instance_get(id, options[i].variable), 0.75, 0.75, 0);
				
				if(mouse_check_button_pressed(mb_left) && point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), 295, 132 + (i * 63), 646, 155	+ (i * _ypad)))
					variable_instance_set(id, options[i].variabledialog, get_string_async(options[i].text, variable_instance_get(id, options[i].variable)));
			} 
			
			draw__button(375, 331, can_connect ? c_white : c_red);
			draw__button(375, 391, c_white);
			...
			draw_text((display_get_gui_width() / 2) - 10, 340, "PLAY");
			draw_text((display_get_gui_width() / 2) - 10, 400, "CREDITS");
	 }
	 else
	 {
		 draw__button(32, 32, c_white);
		 draw_set_color(c_white);
		 draw_set_halign(fa_center);
		 draw_set_valign(fa_middle);
		 draw_set_font(global.bigfont);
		 draw_text(120, 54, "BACK");
		 draw_set_font(global.smallfont2);
		 draw_text(display_get_gui_width() / 2, 170, "MAIN_THING:");
		 draw_text(display_get_gui_width() / 2, 190, "Main programmer");
		 draw_text(display_get_gui_width() / 2, 250, "THESOYSAUCE1:");
		 draw_text_ext(display_get_gui_width() / 2, 280, "Programmed QOL stuff such as the UI and options saving, and helped with testing", 16, 460);
		 draw_text(display_get_gui_width() / 2, 330, "CATNET:");
		 draw_text_ext(display_get_gui_width() / 2, 380, "Programmed QOL stuff such as taunt to swap palettes and characters and going back to the hub instead of the level select, made concepts for the ui, and helped with testing", 16, 400);
	 }
		draw_set_halign(fa_center);
		draw_set_valign(fa_top);
		draw_set_color(c_white);
		draw_set_font(global.smallfont2);
		draw_text(display_get_gui_width() / 2, 40, "PTEM Online GML");
		draw_set_halign(fa_left);
		draw_text(16, display_get_gui_height() - 20, verison);
		
		if (mouse_check_button_pressed(mb_left))
		{
			if !credits
			{
				if (point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), 375, 331, 566, 378) && can_connect)
				{
					can_connect = false;
					network_connect_raw_async(ws, host, port);
				}
				if point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), 375, 391, 566, 438)
					credits = true;
			}
			else if point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), 32, 32, 213, 79)
				credits = false;
		}
	}
	draw_set_color(c_white);
	...
	draw_set_font(global.smallfont2);
	if(connect_time != -1)
		draw_text(0, 0, string(ds_map_size(clients)) + " Online");
		
	for(var len = array_length(notifications), i = len - 1; i >= 0; i--)
	{
		...
		draw_set_alpha(0.6);
		draw_rectangle(notifications[i].x, notifications[i].y, notifications[i].x + 216, notifications[i].y + 79, false);
		draw_set_alpha(1);
		draw_rectangle(notifications[i].x, notifications[i].y, notifications[i].x + 216, notifications[i].y + 71, true);
		
		draw_set_color(c_white);
		
		if notifications[i].duration > 0
		{
			notifications[i].x = lerp(notifications[i].x, 0, 0.09);
			draw_rectangle(notifications[i].x, notifications[i].y + 79, notifications[i].x + 216 * (notifications[i].duration / notifications[i].maxduration), notifications[i].y + 71, false);
		} 
		else
			notifications[i].x = lerp(notifications[i].x, -540, 0.09);
		
		draw_set_color(c_black);
		draw_rectangle(notifications[i].x, notifications[i].y + 71, notifications[i].x + 216, notifications[i].y + 79, true);
		draw_set_color(c_white);
		
		if notifications[i].x < -330
		{
			array_delete(notifications, i, 1);
			continue;
		} 
		
		notifications[i].y = lerp(notifications[i].y, i * 80, 0.1);
		
		draw_set_font(global.smallfont2);
		draw_set_halign(fa_left);
		draw_set_valign(fa_top);
		
		draw_text_ext(notifications[i].x + 16, notifications[i].y + 16, notifications[i].text, 16, 216);
	}
	
	with(obj_player1)
	{
		other.showtext = place_meeting(x, y, obj_startgate);
		draw_set_font(global.bigfont);
		draw_set_halign(fa_center);
		draw_set_valign(fa_top);
		draw_text(display_get_gui_width() / 2 + irandom_range(-1, 1), other.yi, obj_tv.message);
	}
	draw_set_font(-1);
	...
	event.networking[0] = @'
		...
		{
			var message_type = async_load[? "message_type"];
			if(message_type == network_send_binary)
			{
				var buff = async_load[? "buffer"];
				buffer_seek(buff, buffer_seek_start, 0);
				var _id = buffer_read(buff, buffer_u32);
				var _packet_type = buffer_read(buff, buffer_u32);
				switch(_packet_type)
				{
					case 0 /*TYPE_UPDATE*/:
						if(client_id == _id)
						...
						if(buffer_get_size(buff) != 1028)
							return;
							
						if(clients[? _id] == undefined)
						{
							var obj = instance_create(0, 0, obj_custom_object);
							with(obj)
							{
								chat_font = other.chat_font;
								spriteids = 1;
								persistent = 1;
								spr_palette = spr_peppalette;
								paletteselect = 1;
								sprite = spr_player_idle;
								draw_event = other.playerdraw;
								draw_event_saved = other.playerdraw_saved;
								room_ind = undefined;
								image_speed = 0;
								name = "player";
								namecolor = c_white;
							...
							clients[? _id] = [obj, ds_list_create()];
						}
						
						var client = clients[? _id];
						repeat(19)
						{
							var _x = buffer_read(buff, buffer_f32);
							var _y = buffer_read(buff, buffer_f32);
							var _xscale = buffer_read(buff, buffer_u32);
							var _sprite_index = buffer_read(buff, buffer_u32);
							if(!sprite_exists(_sprite_index))
								_sprite_index = real(spr_null);
							var _image_index = buffer_read(buff, buffer_f32);
							var _room = buffer_read(buff, buffer_u32);
							var _spr_palette = buffer_read(buff, buffer_u32);
				if(!sprite_exists(_spr_palette))
								_spr_palette = real(spr_peppalette);
							var _paletteselect = buffer_read(buff, buffer_u32);
							var _image_angle = buffer_read(buff, buffer_f32);
							ds_list_add(client[1], [_x, _y, _xscale, _sprite_index, _image_index, _room, _spr_palette, _paletteselect, _image_angle]);
						}
					break
					
					case 1 /*TYPE_DISCONNECT*/:
						with(clients[? _id][0])
						{
							array_push(other.notifications, 
							{
								text : name + " " + string(_id) + " left the server.",
								duration : 120,
								maxduration : 120, 
								x : -340,
								y : 0
						...
						ds_map_delete(clients, _id);
					break
					
					case 2 /*TYPE_CHAT*/:
						if(buffer_get_size(buff) == 8)
							return;
						var _text = buffer_read(buff, buffer_text);
						var client = clients[? _id];
						...
						if(client != undefined)
						{
							with(client[0])
							{
								array_insert(messages, 0, [_text, 1]);
							}
						}
						array_push(chat_history, client[0].name + " [" + string(_id) + "]: " + _text);
						if(array_length(chat_history) > 1024)
					...
					case 3 /*TYPE_CONNECT*/:
						...
						var buff = buffer_create(0, buffer_grow, 4);
						buffer_seek(buff, buffer_seek_start, 0);
						buffer_write(buff, buffer_u32, 4 /*TYPE_NAMECHANGE*/);
						buffer_write(buff, buffer_u32, string_length(name))
						buffer_write(buff, buffer_text, name);
						network_send_raw(ws, buff, buffer_get_size(buff), network_send_binary);
						buffer_delete(buff);
					break
					
					case 4 /*TYPE_NAMECHANGE*/:
						if(buffer_get_size(buff) == 8)
							return;
						var _name = buffer_read(buff, buffer_text);
						if(clients[? _id] == undefined)
						{
							var obj = instance_create(0, 0, obj_custom_object);
							with(obj)
					...
					case 7 /*TYPE_SPAWN*/:
						...
						var _room = buffer_read(buff, buffer_u32);
						var _text = buffer_read(buff, buffer_text);
						var obj = asset_get_index(_text);
						
						if obj != -1 && _room == real(room)
							instance_create(_x, _y, obj);
					break
					
					case 8 /*TYPE_ADMINABUSE*/:
						if(buffer_get_size(buff) == 8)
							return;
						var client = clients[? _id][0];
						
						if(client.room_ind == room || client.is_self)
						{
							var _text = buffer_read(buff, buffer_text);
		...
		if(type == network_type_non_blocking_connect)
		{
			array_push(notifications, 
			{
				text : async_load[? "succeeded"] ? "Connected to server ip : " + string(async_load[? "ip"]) : "unable to connect " + "to server.", 
				duration : 120,
				maxduration : 120, 
				x : -340,
				y : 0
	...
	event.dialog[0] = @'
		...
		if(i_d == global._chat)
		{
			...
			{
				with(obj_player1) state = 0;
				return;
			}
			
			var _text = async_load[? "result"];
			if(string_pos("/spawn", _text) == 1)
			{
				var _obj = string_copy(_text, 8, string_length(_text));
				var buff = buffer_create(0, buffer_grow, 4);
				buffer_seek(buff, buffer_seek_start, 0);
				buffer_write(buff, buffer_u32, 7 /*TYPE SPAWN*/);
				buffer_write(buff, buffer_u32, string_length(_obj));
				buffer_write(buff, buffer_f32, obj_player1.x);
				buffer_write(buff, buffer_f32, obj_player1.y); 
				buffer_write(buff, buffer_u32, real(room)); 
				buffer_write(buff, buffer_text, _obj);
				network_send_raw(ws, buff, buffer_get_size(buff), network_send_binary);
				buffer_delete(buff);
				with(obj_player1) state = 0;
				return;
			}
			if(string_pos("/disconnect", _text) == 1)
			{
				with(obj_player1) state = 0;
				alarm[0] = 1;
				return;
			}
			if(string_pos("/ch", _text) == 1)
			{
				with(obj_player1) state = 0;
				var _ = "";
			...
			if(string_pos("/teleportto ", _text) == 1)
			{
				with(obj_player1) state = 0;
				var _target = string_copy(_text, 13, string_length(_text));
				var _keys = ds_map_keys_to_array(clients);
				var _self = false;
				for(var i = 0; i < array_length(_keys); i++)
				{
					...
					with(client[0])
					{
						if(string_pos(_target, name + string(_keys[i])) == 0)
							continue;
						if(is_self)
						{
							_self = true;
				...
				array_push(notifications, 
				{
					text : _self ? "You can`t teleport to yourself silly!": "No client found matching \"" + _target + "\"", 
					duration : 120,
					maxduration : 120, 
					x : -340,
					y : 0
		...
		if(string_pos("/login ", _text) == 1)
			_text = "/login " + md5_string_utf8(string_copy(_text, 8, string_length(_text)));
			var buff = buffer_create(0, buffer_grow, 4);
			buffer_seek(buff, buffer_seek_start, 0);
			buffer_write(buff, buffer_u32, 2 /*TYPE_CHAT*/);
			buffer_write(buff, buffer_u32, string_length(_text))
			buffer_write(buff, buffer_text, _text);
			network_send_raw(ws, buff, buffer_get_size(buff), network_send_binary);
			buffer_delete(buff);
			with(obj_player1) state = 0;
		}
		...
		if(i_d == namedialog)
		{
			...
			name = string_replace_all(string_copy(async_load[? "result"], 0, 24), "\n", "");
		...
		if(i_d == hostdialog)
		{
			...
			host = async_load[? "result"];
			ini_write_string("data", "host", host);
			ini_close();
		}
		...
		if(i_d == portdialog)
		{
			...
			try
			{
				port = int64(async_load[? "result"]);
			}
			catch(e){}
			ini_write_real("data", "port", port);
			ini_close();
		}
	';
	event.draw[0] = @'
	...
	alarm[5] = 1;
	/*
		PTEM ONLINE GML VERSION 1.0
		CREDITS:
		main_thing -- Main Programmer
		thesoysauce1 -- Programmed QOL stuff such as the UI and options saving, and helped with testing
		catnet -- Programmed changing characters and palettes through the taunt, made concepts for the ui, and helped with testing
		punch -- cuz theyre cool
		
		1. Replace NAME with your username
		2. Replace SERVER with the IP thats provided by the host
		3. To use chat, bind the following command
		bind gml global._chat=get_string_async("chat", "");with(obj_player1){state=1;sprite_index=spr_phoneidle}
		4. To leave the server type /disconnect in chat or just close the game
	*/
	
	/**** DEFAULT VALUES ****/
	#macro NAME "player"
	#macro SERVER "ws://127.0.0.1/"
	#macro SERVER_PORT 6789
	// Set IS_SYSTEM_FONT to false if you want to set CHAT_FONT
	// to something like global.smallfont2
	#macro IS_SYSTEM_FONT true
	#macro CHAT_FONT_SIZE 12
	#macro CHAT_FONT font_add("/system/fonts/Roboto-Regular.ttf", CHAT_FONT_SIZE, false, false, 32, 256)
	// ord("A")
	#macro CHAT_BIND noone
}